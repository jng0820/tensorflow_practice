...
keep_prop = tf.Variable(tf.float32)
...
L4 = tf.nn.dropout(L4,keep_prop=keep_prop)
...
for epoch in range(training_epoch):
    ...
    for i in range(total_batch):
        ...
        c, _, = sess.run([cost,optimizer],feed_dict={X:batch_xs, Y:batch_ys,keep_prop : 0.75})
...
print('Accuracy : ', sess.run(accruacy,feed_dict={X:mnist.test.images, Y:mnist.test.labels,keep_prop : 1}))